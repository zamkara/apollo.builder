        ...
        std::thread::spawn(move || {
            ...
            rt.block_on(async {
                let bootc_cmd = format!(
                    "DISK=\"{}\"; VARIANT=\"{}\"; ZRAM_VAL=\"{}\"; killall -9 bootc skopeo 2>/dev/null || true; for p in $DISK*; do umount -l $p 2>/dev/null || true; done; umount -l /run/bootc/mounts/rootfs 2>/dev/null || true; btrfs device scan --forget 2>/dev/null || true; wipefs -af $DISK* 2>/dev/null || true; bootc install to-disk --wipe --filesystem btrfs --bootloader none --source-imgref docker://$VARIANT $DISK && sgdisk -c 1:\"AETHER\" -c 2:\"mbut\" -c 3:\"endfield\" $DISK && fatlabel \"$DISK\"1 AETHER || true; e2label \"$DISK\"2 mbut || true; xfs_admin -L mbut \"$DISK\"2 || true; btrfs filesystem label \"$DISK\"3 endfield || true; mount \"$DISK\"3 /mnt && DEPLOY_ETC=$(ls -d /mnt/ostree/deploy/default/deploy/*/etc | head -n 1) && mkdir -p $DEPLOY_ETC/systemd && if [ \"$ZRAM_VAL\" != \"disabled\" ]; then echo \"[zram0]\" > $DEPLOY_ETC/systemd/zram-generator.conf; echo \"compression-algorithm = zstd\" >> $DEPLOY_ETC/systemd/zram-generator.conf; if [ \"$ZRAM_VAL\" = \"auto\" ]; then echo \"zram-size = ram / 2\" >> $DEPLOY_ETC/systemd/zram-generator.conf; else echo \"zram-size = $ZRAM_VAL\" >> $DEPLOY_ETC/systemd/zram-generator.conf; fi; else rm -f $DEPLOY_ETC/systemd/zram-generator.conf; fi && umount -l /mnt", 
