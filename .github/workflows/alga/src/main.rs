        ...
        std::thread::spawn(move || {
            ...
            rt.block_on(async {
                let bootc_cmd = format!(
                    "DISK=\"{}\"; VARIANT=\"{}\"; ZRAM_VAL=\"{}\"; killall -9 bootc skopeo 2>/dev/null || true; for p in $DISK*; do umount -l $p 2>/dev/null || true; done; umount -l /run/bootc/mounts/rootfs 2>/dev/null || true; btrfs device scan --forget 2>/dev/null || true; wipefs -af $DISK* 2>/dev/null || true; mkdir -p /etc/ostree && echo \"[sysroot]\" > /etc/ostree/prepare-root.conf && bootc install to-disk --wipe --filesystem btrfs --bootloader none --source-imgref docker://$VARIANT $DISK && sgdisk -c 1:\"AETHER\" -c 2:\"mbut\" -c 3:\"endfield\" $DISK && part() {{ if [[ $1 == *[0-9] ]]; then echo \"${{1}}p$2\"; else echo \"${{1}}$2\"; fi; }}; P1=$(part $DISK 1); P2=$(part $DISK 2); P3=$(part $DISK 3); udevadm settle; fatlabel $P1 AETHER || true; e2label $P2 mbut || true; xfs_admin -L mbut $P2 || true; btrfs filesystem label $P3 endfield || true; mount $P3 /mnt && DEPLOY_ETC=$(ls -d /mnt/ostree/deploy/default/deploy/*/etc | head -n 1) && mkdir -p $DEPLOY_ETC/systemd && if [ \"$ZRAM_VAL\" != \"disabled\" ]; then echo \"[zram0]\" > $DEPLOY_ETC/systemd/zram-generator.conf && echo \"compression-algorithm = zstd\" >> $DEPLOY_ETC/systemd/zram-generator.conf && if [ \"$ZRAM_VAL\" = \"auto\" ]; then echo \"zram-size = ram / 2\" >> $DEPLOY_ETC/systemd/zram-generator.conf; else echo \"zram-size = $ZRAM_VAL\" >> $DEPLOY_ETC/systemd/zram-generator.conf; fi; else rm -f $DEPLOY_ETC/systemd/zram-generator.conf; fi && umount -l /mnt", 
                    disk, variant, zram_val
                );
                ...
                match status {
                    Ok(s) if s.success() => {
                        ...
                        let bootloader_cmd = format!(
                            "EFI_PART=$(lsblk -rno PATH,PARTTYPE {} | grep -i 'c12a7328-f81f-11d2-ba4b-00a0c93ec93b' | head -n1 | awk '{{print $1}}'); BOOT_PART=$(lsblk -rno PATH,PARTTYPE {} | grep -i 'bc13c2ff-59e6-4262-a352-b275fd6f7172' | head -n1 | awk '{{print $1}}'); ROOT_PART=$(lsblk -rno PATH,PARTTYPE {} | grep -i '4f68bce3-e8cd-4db1-96e7-fbcaf984b709' | head -n1 | awk '{{print $1}}'); if [ -n \"$EFI_PART\" ] && [ -n \"$BOOT_PART\" ] && [ -n \"$ROOT_PART\" ]; then mkdir -p /tmp/efi_mnt /tmp/boot_mnt /tmp/root_mnt; umount -l $EFI_PART 2>/dev/null || true; umount -l $BOOT_PART 2>/dev/null || true; umount -l $ROOT_PART 2>/dev/null || true; mount $BOOT_PART /tmp/boot_mnt; mount $ROOT_PART /tmp/root_mnt; mount $EFI_PART /tmp/efi_mnt && bootctl install --esp-path=/tmp/efi_mnt && mkdir -p /tmp/efi_mnt/ostree && cp -r /tmp/boot_mnt/ostree/* /tmp/efi_mnt/ostree/ && mkdir -p /tmp/efi_mnt/loader/entries && cp /tmp/boot_mnt/loader/entries/*.conf /tmp/efi_mnt/loader/entries/ && sed -i 's|^linux /boot/ostree|linux /ostree|g' /tmp/efi_mnt/loader/entries/*.conf && sed -i 's|^initrd /boot/ostree|initrd /ostree|g' /tmp/efi_mnt/loader/entries/*.conf && sed -i 's/bootloader=none/bootloader=systemd-boot/' /tmp/root_mnt/ostree/repo/config {} && umount /tmp/efi_mnt && umount /tmp/boot_mnt && umount /tmp/root_mnt; fi",
                            disk, disk, disk, if install_grub { "&& grub-install --target=x86_64-efi --efi-directory=/tmp/efi_mnt --bootloader-id=arkGRUB --recheck 2>/dev/null || true" } else { "" }
                        );
                        let _ = tokio::process::Command::new("pkexec")
