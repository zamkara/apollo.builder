...
This document outlines the standard Git workflow and repository configuration for ark linux. Adhering to these guidelines is critical to maintaining build pipeline integrity and preventing desynchronization.
...
**Rationale:** The ark linux Continuous Integration (CI) pipeline relies exclusively on GitHub Actions. The automated processes that construct the container image, compile the Alga installer, and generate the final bootable `.iso` artifact are triggered by commits pushed to the GitHub repository. Pushing changes to the GitLab remote will bypass the CI pipeline entirely, resulting in no updated ISO being generated.
