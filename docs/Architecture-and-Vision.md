...
This document outlines the core architecture and development philosophy of Ark OS, providing a foundational understanding for all contributors.

## Core Vision
Ark OS is designed as a modern, image-based operating system. The core philosophy centers on immutability, reliable rollbacks, and a container-native distribution model. The objective is to provide a system where the root filesystem is read-only by default, ensuring that the OS state remains predictable and identical to the tested build artifacts.

## System Anatomy

Ark OS is built upon three primary components:

### 1. OSTree and Container-Native Deployments (bootc)
Ark OS is distributed as an OCI container image rather than a traditional package repository. 
...
Instead of relying on generic installers like Calamares, Ark OS utilizes a custom-built installer named **Alga**.
...
The entire OS generation process is automated. The base image resides at `ghcr.io/zamkara/ark.linux-nvidia:latest`. The `ark.builder` repository utilizes a `Containerfile` to layer local dependencies (e.g., AUR packages) on top of the base image, compiles the Alga installer, and relies on GitHub Actions to securely package the final `.iso` artifact.

## Architectural Advantages
By adopting a container-native approach, Ark OS eliminates traditional dependency resolution failures on the client side. If a package build fails, it fails in the GitHub Actions pipeline, preventing broken images from ever reaching the end user.
