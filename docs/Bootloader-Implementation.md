...
This document details the engineering decisions behind ark OS's bootloader installation process, specifically the shift from `bootupd` to a native `bootctl` approach.
...
ark OS is based on **Arch Linux**, which ships standard, unsigned `systemd-boot` EFI binaries (`systemd-bootx64.efi`). 
...
To ensure a robust and reliable boot sequence, ark OS completely bypasses the incompatible `bootupd` implementation in favor of a native approach.
...
**Result:** ark OS utilizes a pure, native `systemd-boot` implementation that fully complies with the Boot Loader Specification (BLS) and integrates seamlessly with OSTree deployments without relying on fragile workarounds.
